...
const Create: React.FC = () => {
	const { DrawerCreate, RouteAPI, isLoading, setLoading, setSnackMessage, setSnackbarOpen } = useEscolasContext();
	...
	const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (Classes.length > 0) {
			Classes?.forEach((addingClass) => {
				...
				RequestBody.turmas.push({ nome: ClassName?.value, idCurso: ClassYear?.value });
			});
		}

		try {
			await RouteAPI.post("/escola", RequestBody);
			setSnackMessage("Escola criada com sucesso");
		...
		<Drawer anchor="right" open={DrawerCreate.situation} onClose={onClose}>
			<div className={styles.insert_container}>
				...
				<footer className={styles.senders}>
					...
					<button className={styles.submiter} type="submit" form="create-school">
